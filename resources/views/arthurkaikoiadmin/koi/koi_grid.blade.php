				...
				<div class="content_box"><a href="{{ route('cmskoidetail', $k->id_koi) }}">
                    ...
                    @if ($loop->first)
                     ...
                     @endforeach
                        ...
                        <h6 style="padding-left: 10px; padding-right: 10px; padding-top: 10px;"><a href="{{ route('cmskoidetail', $k->id_koi) }}"> {{ $k->koi_code }}</a></h5>
                        ...
                        <div class="item_add" style="padding-left: 10px; padding-right: 10px; padding-bottom: 25px; padding-top: 25px; display: inline-flex;">
                            <span style="display: flex !important;">
                                <a href="{{ route('cmskoigEdit', $k->id_koi) }}"
                                    class="btn btn-warning btn-xs" style="width: 30px"><i class="fas fa-edit"></i></a> |
                                <a href="#bannerformmodal{{ $k->id_koi }}"
                                    class="btn btn-danger btn-xs" data-toggle="modal" data-target="#modal-danger{{ $k->id_koi }}" style="color: black; width: 30px"><i class="far fa-trash-alt"></i></a> |
                                <a href="{{ route('cmskoidetailgrid', $k->id_koi) }}"
                        ...
                        <div class="item_add" style="padding-left: 10px; padding-right: 10px; padding-bottom: 25px; padding-top: 25px; display: inline-flex;">
                            <span style="display: flex !important;">
                                <a href="{{ route('cmskoidetailgrid', $k->id_koi) }}" class="btn btn-xs"
                                ...
                                <a href="{{ route('cmskoidetail', $k->id_koi) }}"
                                    ...
                                    data-target="#modalShowDocument{{ $k->id_koi }}" style="margin-left: 5px; width 30px;"><i class="fas fa-photo-video"></i></a>
                                <!-- modalShowDocument -->
                                <div class="modal fade bannerformmodal{{ $k->id_koi }}"
            ...
            <ul class="pagination justify-content-center m-0">
                @if ($koi->lastPage() > 1)
                    ...
                    @if ($koi->currentPage() > 3)
                        <li class="page-item"><a class="page-link" href="{{ $koi->url(1) }}">1</a></li>
                    ...
                    @for ($i = max($koi->currentPage() - 2, 1); $i <= min($koi->currentPage() + 2, $koi->lastPage()); $i++)
                        <li class="page-item {{ ($koi->currentPage() == $i) ? 'active' : '' }}">
                            <a class="page-link" href="{{ $koi->url($i) }}">{{ $i }}</a>
                        </li>
                    @endfor
                    ...
                    @if ($koi->currentPage() < $koi->lastPage() - 2)
                        @if ($koi->currentPage() < $koi->lastPage() - 3)
                            <li class="page-item disabled"><span class="page-link">...</span></li>
                        @endif
                        <li class="page-item"><a class="page-link" href="{{ $koi->url($koi->lastPage()) }}">{{ $koi->lastPage() }}</a></li>
                    @endif
                @endif
            </ul>
        </nav>
